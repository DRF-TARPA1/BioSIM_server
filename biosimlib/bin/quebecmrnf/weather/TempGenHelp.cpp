...
ERMsg CTempGenHelp::Initialize( const char* DLLFilePath)
{
    ...
    //load dll
    m_hDll = LoadLibrary(DLLFilePath);
...
// Note:       
//****************************************************************************
void CTempGenHelp::SetTarget( const char* name, const double& lat, const double& lon, float elev, float slope, float orientation)
{
    _ASSERTE(m_SetTarget!= NULL);
    
    m_SetTarget(name, lat, lon, elev, slope, orientation);
}
...
//****************************************************************************
void CTempGenHelp::SetReplication( short nbRep, bool bFixedSeed )
{
    _ASSERTE(m_SetReplication!= NULL);
    
    return m_SetReplication(nbRep, bFixedSeed);
}
                    ...
                    short albedoType, const char* category)
{
    ...
    return m_SetTGInput(firstYear, lastYear, nbNormalStation, nbDailyStation, albedoType, category);
}
...
//****************************************************************************
ERMsg CTempGenHelp::Save(const CStdStringVector& outputFilePathVector)const
{
	...
	for(int i=0; i<(int)outputFilePathVector.size(); i++)
	{
		...
		strcpy( pOutputFile[i], outputFilePathVector[i] );
	}
